  ...
  <el-main class="bg table_wrap">
      ...
      <el-row class="rows row2">
        <el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
          <el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
            ...
            <el-button
              v-if="user_group == '管理员' || $check_action('/homestay_reservation/table', 'add') || $check_action('/homestay_reservation/view', 'add')"
              @click="$router.push('./view?')" class="add">添加</el-button>

            <el-button
              v-if="user_group == '管理员' || $check_action('/homestay_reservation/table', 'del') || $check_action('/homestay_reservation/view', 'del')"
    ...
    <el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%"
      ...
      <el-table-column fixed="right" label="操作" min-width="120"
        v-if="user_group == '管理员' || $check_action('/homestay_reservation/table', 'set') || $check_action('/homestay_reservation/view', 'set') || $check_action('/homestay_reservation/view', 'get') || $check_action('/申请退款/table', 'add') || $check_action('/申请退款/view', 'add')">
        ...
        <template slot-scope="scope">
          ...
          <el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;"
            size="small" @click="to_table(scope.row, '/refunds_for_homestays/view')"
            v-if="user_group == '管理员' || $check_action('/refunds_for_homestays/table', 'add') || $check_action('/refunds_for_homestays/view', 'add')">
            <span>申请退款</span>
          </el-button>
          <el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;"
            size="small" @click="openPayModal(scope.row)"
            v-if="scope.row.pay_state === '未支付' && (user_group == '管理员' || $check_pay('/homestay_reservation/table'))">
            <span>支付</span>
          </el-button>
        </template>
      </el-table-column>
...
*/
    ...
    openPayModal(v) {
      ...
      console.log("33", v)
      this.pay_obj.id = v.homestay_reservation_id
      let homestay_name = v.homestay_name
      let pay_money = v.appointment_amount

      window.open("http://localhost:5000/api/homestay_reservation/pay?subject=" + homestay_name + "&traceNo=" + v.homestay_reservation_id + "&totalAmount=" + pay_money)
