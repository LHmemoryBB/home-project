<template>
	<div class="diy_details_box diy_div_forum_sharing">
		<div class="warp diy_warp">
			...
			<div class="details_btn_wrap">
																<button class="btn btn-primary details_btn" v-if="$check_action('/forum_sharing/details','set')" @click="change_praise()" ><span>点赞</span></button>
																<button class="btn btn-primary details_btn" v-if="$check_action('/forum_sharing/details','set')" @click="change_collect()" ><span>收藏</span></button>
											</div>
						<!-- 富文本 -->
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "forum_sharing",
					source_field: "forum_sharing_id",
					source_id: this.obj["forum_sharing_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/forum_sharing/set?forum_sharing_id=' + _this.obj["forum_sharing_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.share_title
												query.title = this.obj.sharing_categories
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var forum_sharing_id = this.obj.forum_sharing_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "forum_sharing",
          source_field: "forum_sharing_id",
          source_id: forum_sharing_id
        }, (res) => {
