<template>
	<div class="diy_form">
			...
			<b-form-select id="input-${idx}" v-model="form['user_gender']" :options="getDropList(list_user_gender)"></b-form-select>
				...
				<b-form-group v-if="$check_register_field('add','user_age','/regular_users/view')" id="input-group-${idx}" label="用户年龄" label-for="input-${idx}">
			<b-form-input id="input-${idx}" v-model="form['user_age']" type="text" placeholder="用户年龄" trim></b-form-input>
		</b-form-group>
				<b-form-group v-if="$check_register_field('add','user_points','/regular_users/view')" id="input-group-${idx}" label="用户积分" label-for="input-${idx}">
